...
export default function Sidebar() {
  ...
  return (
    <aside className="flex flex-col h-screen bg-white text-gray-800 px-4 py-6 border-r border-gray-100 shadow-sm">
      ...
      {/* Profile Card */}
      <div className="mx-2 mb-6 px-4 py-3 rounded-xl border border-[#94e5ec] bg-[#94e5ec]/10">
        <div className="flex items-center gap-3">
          <img src={profile?.profile_pic} alt="profile_pic" className="w-[30px] rounded-full" />
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {sidebarLinks.map(({ id, to, icon, label, end }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-2 rounded-lg mt-5 font-medium transition-all duration-200
              ${isActive
